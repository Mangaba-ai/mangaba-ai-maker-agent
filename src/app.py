...
def enhanced_detect_goal_type(goal: str, context: str = "") -> str:
    ...
    # Análise de concorrência (PRIORIDADE ALTA - deve vir antes de sales_analysis)
    if any(word in combined_text for word in ['análise de concorrência', 'concorrência', 'concorrentes', 'benchmarking', 'análise competitiva', 'mercado competitivo', 'posicionamento mercado', 'competitivo', 'competitor', 'market share', 'participação mercado', 'inteligência competitiva']):
        return 'competitive_analysis'
    
    # Planejamento estratégico (PRIORIDADE ALTA)
    elif any(word in combined_text for word in ['planejamento estratégico', 'estratégia empresarial', 'plano estratégico', 'visão estratégica', 'missão', 'valores', 'objetivos estratégicos', 'metas organizacionais', 'okr', 'okrs', 'objectives and key results', 'swot', 'balanced scorecard', 'kpi estratégico']):
        return 'strategic_planning'
    
    # Análise de vendas e performance comercial
    elif any(word in combined_text for word in ['vendas', 'receita', 'faturamento', 'conversão', 'pipeline', 'crm', 'leads', 'prospects', 'clientes', 'ticket médio', 'ltv', 'churn', 'funil vendas', 'performance comercial', 'roi vendas', 'forecast']):
        return 'sales_analysis'
    
    # Gestão de produtos
    elif any(word in combined_text for word in ['produto', 'produtos', 'roadmap', 'features', 'backlog', 'mvp', 'user story', 'product owner', 'desenvolvimento produto', 'lançamento', 'product market fit', 'ciclo vida produto']):
        return 'product_management'
    
    # Gestão de usuários e experiência
    elif any(word in combined_text for word in ['usuários', 'users', 'ux', 'ui', 'experiência usuário', 'jornada usuário', 'personas', 'segmentação', 'comportamento usuário', 'usabilidade', 'customer journey', 'user research']):
        return 'user_management'
    
    # Gestão de tarefas e projetos
    elif any(word in combined_text for word in ['tarefas', 'tasks', 'sprint', 'scrum', 'kanban', 'agile', 'projeto', 'cronograma', 'milestone', 'deliverables', 'gestão projetos', 'pmo', 'waterfall']):
        return 'task_management'
    
    # Análise financeira e orçamentária
    elif any(word in combined_text for word in ['financeiro', 'orçamento', 'budget', 'fluxo caixa', 'dre', 'balanço', 'roi', 'investimento', 'custo', 'margem', 'lucro', 'ebitda', 'valuation']):
        return 'financial_analysis'
    
    # Recursos humanos e gestão de pessoas
    elif any(word in combined_text for word in ['recursos humanos', 'rh', 'colaboradores', 'funcionários', 'recrutamento', 'seleção', 'treinamento', 'desenvolvimento', 'performance', 'avaliação desempenho', 'cultura organizacional']):
        return 'hr_management'
    
    # Marketing e comunicação
    elif any(word in combined_text for word in ['marketing', 'campanha', 'comunicação', 'branding', 'marca', 'publicidade', 'digital marketing', 'seo', 'sem', 'social media', 'content marketing', 'inbound']):
        return 'marketing_analysis'
    
    # Operações e processos
    elif any(word in combined_text for word in ['operações', 'processos', 'workflow', 'automação', 'eficiência', 'produtividade', 'lean', 'six sigma', 'melhoria contínua', 'otimização']):
        return 'operations_management'
    
    # Tecnologia e inovação
    elif any(word in combined_text for word in ['tecnologia', 'inovação', 'digital', 'transformação digital', 'ti', 'sistemas', 'software', 'infraestrutura', 'arquitetura', 'desenvolvimento']):
        return 'technology_analysis'
    
    # Tipos acadêmicos/científicos
    elif any(word in combined_text for word in ['artigo', 'paper', 'pesquisa', 'estudo', 'análise científica', 'metodologia', 'revisão bibliográfica', 'tese', 'dissertação', 'monografia']):
        return 'academic'
    
    # Relatórios técnicos
    elif any(word in combined_text for word in ['relatório', 'report', 'análise técnica', 'diagnóstico', 'avaliação', 'auditoria', 'compliance']):
        return 'technical_report'
    
    # Resumos e sínteses
    elif any(word in combined_text for word in ['resumo', 'resumir', 'sintetizar', 'síntese', 'sumarizar', 'executive summary']):
        return 'summary'
    
    # Análise de dados
    elif any(word in combined_text for word in ['dados', 'estatística', 'gráfico', 'dashboard', 'métricas', 'kpi', 'analytics', 'business intelligence', 'big data', 'data science']):
        return 'data_analysis'
    
    # Documentação
    elif any(word in combined_text for word in ['documentação', 'manual', 'guia', 'tutorial', 'procedimento', 'política', 'norma', 'regulamento']):
        return 'documentation'
    
    # Planejamento geral
    elif any(word in combined_text for word in ['plano', 'planejamento', 'cronograma', 'projeto']):
        return 'planning'
    
    # Criativo/Marketing
    elif any(word in combined_text for word in ['campanha', 'marketing', 'criativo', 'conteúdo', 'post', 'artigo blog']):
        return 'creative'
